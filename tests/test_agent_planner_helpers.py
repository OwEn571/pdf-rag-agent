...
from app.services.agent.planner_helpers import (
    ...
    plan_prefers_premature_human_clarification,
...
def test_planner_searches_recovered_research_intent_before_clarifying() -> None:
    settings = SimpleNamespace(confidence_floor=0.6)
    contract = QueryContract(
    ...
    assert research_contract_should_try_tools_before_human(contract) is True
    assert should_fallback_to_human(contract=contract, settings=settings) is False
    assert fallback["actions"] == []
    assert plan_prefers_premature_human_clarification(contract=contract, plan=premature_plan) is True
    assert defer_premature_research_clarification(
        contract=contract,
        plan=premature_plan,
        fallback=fallback,
    )["actions"] == []
