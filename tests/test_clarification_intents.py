...
from app.services.clarification.intents import (
    AMBIGUITY_DETECTION_STAGE,
    AMBIGUITY_RESOLUTION_STAGE,
...
def test_resolve_disambiguation_judge_decision_auto_resolves_contract() -> None:
    ...
    assert result.observation_tool == "compose"
    assert result.observation_summary == "options=2, judge=auto_resolve, confidence=0.91"
    assert result.observation_payload["stage"] == AMBIGUITY_RESOLUTION_STAGE
...
def test_resolve_disambiguation_judge_decision_marks_clarification_with_recommendation() -> None:
    ...
    assert result.observation_tool == "ask_human"
    assert result.observation_payload["stage"] == AMBIGUITY_DETECTION_STAGE
