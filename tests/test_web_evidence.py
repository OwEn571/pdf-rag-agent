...
from app.services.retrieval.web_evidence import (
    ...
    should_add_web_claim_from_context,
    solve_claims_with_web_research,
    web_include_domains,
    web_include_domains_from_context,
    web_query_text,
    web_query_text_from_context,
    web_search_topic,
)
...
def test_web_evidence_accepts_typed_research_plan_context() -> None:
    context = ResearchPlanContext(
        clean_query="最新 RAG 论文",
        targets=("RAG",),
        requested_fields=("recommended_papers",),
        goals=frozenset({"recommended_papers"}),
        required_modalities=("page_text",),
    )

    assert web_query_text_from_context(context) == "最新 RAG 论文 paper arXiv publication"
    assert "arxiv.org" in web_include_domains_from_context(context)
    assert should_add_web_claim_from_context(
        context=context,
        claims=[Claim(claim_type="summary", entity="RAG", value="local")],
        explicit_web=False,
