...
from app.services.pdf_rendering import subprocess_command_allowed
...
def test_agent_subprocess_allowlist_only_allows_bare_pdftoppm() -> None:
    assert subprocess_command_allowed(["pdftoppm", "-png"]) is True
    assert subprocess_command_allowed(["/usr/bin/pdftoppm", "-png"]) is False
    assert subprocess_command_allowed(["python", "-c", "print(1)"]) is False
    assert subprocess_command_allowed([]) is False
