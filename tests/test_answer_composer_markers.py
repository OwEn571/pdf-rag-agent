...
from app.services.answers.formula import FORMULA_ANSWER_MARKERS, format_formula_symbol
from app.services.answers.topology import UNUSABLE_TOPOLOGY_RECOMMENDATION_MARKERS


def test_clean_topology_public_text_uses_blocked_marker_profile() -> None:
    assert "cannot determine" in UNUSABLE_TOPOLOGY_RECOMMENDATION_MARKERS
...
def test_format_formula_symbol_uses_math_symbol_marker_profile() -> None:
    assert "formula_math_symbol" in FORMULA_ANSWER_MARKERS
    assert format_formula_symbol(r"\pi_\theta") == r"$\pi_\theta$"
    assert format_formula_symbol("∇") == "$∇$"
