...
from app.services.intents.research import (
...
def test_research_intent_classifier_detects_origin_metric_and_summary() -> None:
    ...
    assert looks_like_origin_lookup_query("DeepSeek 是哪篇论文？")
    assert looks_like_origin_lookup_query("DeepSeek 的具体论文来源")
...
def test_research_answer_slots_cover_common_research_queries() -> None:
    ...
    assert research_answer_slots(
        clean_query="AlignX主要结论是什么？用什么数据支持？",
        lowered="alignx主要结论是什么？用什么数据支持？",
        compact="alignx主要结论是什么？用什么数据支持？",
    ) == ["paper_summary"]
    assert research_answer_slots(
        clean_query="GRPO、PPO、DPO三者的区别和联系是什么",
        lowered="grpo、ppo、dpo三者的区别和联系是什么",
        compact="grpo、ppo、dpo三者的区别和联系是什么",
    ) == ["comparison"]
