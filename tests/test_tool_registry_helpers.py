...
def test_tool_registry_helpers_record_tool_observation() -> None:
    ...
    assert events == [("observation", {"tool": "read_memory", "summary": "turns=1", "payload": payload})]
    assert steps == [{"node": "agent_tool:read_memory", "summary": "turns=1"}]
...
def test_tool_registry_helpers_store_conversation_answer_result() -> None:
    ...
    assert events == [("answer_delta", {"text": "hello"})]
...
def test_tool_registry_helpers_build_read_memory_payload(tmp_path: Path) -> None:
...
def test_tool_registry_helpers_build_propose_tool_payload(tmp_path: Path) -> None:
    agent = SimpleNamespace(
        ...
        agent_settings=SimpleNamespace(dynamic_tool_deployment_id="prod-east"),
    )
    ...
    assert payload["scope"]["deployment_id"] == "prod-east"
