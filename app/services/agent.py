...
class ResearchAssistantAgentV4(
    ...
    def _run_library_citation_ranking_if_needed(
        self,
        *,
        query: str,
        session_id: str,
        ...
        if not self._is_citation_ranking_query(clean_query):
            return None
        if not self._citation_ranking_has_library_context(clean_query=clean_query, session=session):
            return None

        candidates = self._select_citation_ranking_candidates(session=session, query=clean_query, limit=6)
        contract = QueryContract(
            clean_query=clean_query,
            interaction_mode="conversation",
            relation="library_citation_ranking",
            targets=[item["title"] for item in candidates[:6]],
            ...
            notes=["external_metric", "citation_count_requires_web", "no_default_recommendation_fallback"],
        )
        plan_summary = {
            "thought": "Treat the message as a criterion refinement of the previous library recommendation, then use external citation evidence before ranking.",
            "actions": [
        ...
        execution_steps.append({"node": "citation_rank_planner", "summary": f"candidates={len(candidates)}"})
        ...
        def publish(text: str) -> None:
            ...
            for chunk in self._chunk_text(text, size=96):
                emit("answer_delta", {"text": chunk})
        emit(
            "compound_task",
            {
                "index": 1,
                "relation": "citation_count_lookup",
                "query": clean_query,
                "candidate_count": len(candidates),
            },
        )
        lookup = self._lookup_candidate_citation_counts(
            candidates=candidates,
            max_web_results=max_web_results,
            emit=emit,
            execution_steps=execution_steps,
        )
        answer_body = self._format_citation_ranking_answer(
            candidates=candidates,
            citation_results=lookup["results"],
            web_enabled=lookup["web_enabled"],
        )
        publish(answer_body)
        ...
        if not citation_doc_ids and evidence:
            citation_doc_ids = [item.doc_id for item in evidence[:3]]
        citations = self._dedupe_citations(self._citations_from_doc_ids(citation_doc_ids, evidence))

        session.last_relation = "library_citation_ranking"
        ranked_titles = [item["title"] for item in candidates[:6]]
        active_research = self._make_active_research(
            relation="library_citation_ranking",
            targets=ranked_titles,
        ...
        counted = [item for item in lookup["results"] if item.get("citation_count") is not None]
        response = AssistantResponse(
            session_id=session_id,
            interaction_mode="conversation",
            answer=answer,
    ...
    def _run_memory_synthesis_if_needed(
        self,
        *,
        query: str,
        session_id: str,
        ...
        clean_query = " ".join(query.strip().split())
        if not self._is_memory_synthesis_query(clean_query):
            return None
        bindings = self._active_memory_bindings(session)
        last_compound = dict((session.working_memory or {}).get("last_compound_query", {}) or {})
        subtasks = list(last_compound.get("subtasks", []) or [])
        if len(bindings) < 2 and len(subtasks) < 2:
            return None

        targets = [str(item.get("target", "") or "").strip() for item in bindings]
        active = session.effective_active_research()
        targets = list(dict.fromkeys([item for item in targets if item] or list(active.targets)))
        titles = list(dict.fromkeys(str(item.get("title", "") or "").strip() for item in bindings if str(item.get("title", "") or "").strip()))
        contract = QueryContract(
            clean_query=clean_query,
            interaction_mode="conversation",
            ...
            notes=["conversation_memory_synthesis"],
        )
        plan_summary = {
            "thought": "Use retained conversation memory as the evidence source for this follow-up synthesis.",
            "actions": ["read_conversation_memory", "synthesize_previous_results", "compose_or_ask_human"],
            "stop_conditions": ["answer_uses_only_memory"],
        }
        emit("contract", contract.model_dump())
        emit("agent_plan", plan_summary)
        self._record_agent_observation(
            ...
            summary=f"bindings={len(bindings)}, subtasks={len(subtasks)}",
            payload={"targets": targets, "titles": titles[:4]},
        )
        answer = self._compose_memory_synthesis_answer(query=clean_query, session=session, contract=contract)
        emit("answer_delta", {"text": answer})
        citations = self._citations_from_memory_bindings(bindings)
        session.last_relation = contract.relation
        active_research = self._make_active_research(
            relation=contract.relation,
            targets=targets or list(active.targets),
            titles=titles or list(active.titles),
            requested_fields=list(contract.requested_fields),
            required_modalities=[],
            answer_shape=contract.answer_shape,
            precision_requirement=contract.precision_requirement,
        ...
        self.sessions.commit_turn(
            ...
            SessionTurn.from_contract(
                ...
                contract=contract,
                interaction_mode="conversation",
                targets=list(active_research.targets),
        ...
        response = AssistantResponse(
            ...
            runtime_summary=self._runtime_summary(
                ...
                tool_plan=plan_summary,
                execution_steps=execution_steps,
                verification_report={"status": "pass", "recommended_action": "memory_synthesis"},
                citations=citations,
            ),
            execution_steps=execution_steps,
            verification_report={"status": "pass", "recommended_action": "memory_synthesis"},
    ...
    def _run_compound_query_if_needed(
        self,
        *,
        query: str,
        session_id: str,
        ...
        if clarification_choice is not None:
            return None
        clean_query = " ".join(query.strip().split())
        if not self._should_try_compound_decomposition(clean_query, session=session):
            return None
        subcontracts = self._llm_decompose_compound_query(clean_query=clean_query, session=session)
        subcontracts = self._merge_redundant_field_subtasks(subcontracts)
        ...
        for index, sub_contract in enumerate(subcontracts, start=1):
            ...
            if sub_contract.relation == "comparison_synthesis":
                comparison = self._compose_compound_comparison_answer(
                    query=clean_query,
                    subtask_results=subtask_results,
                    session=session,
                    comparison_contract=sub_contract,
                )
                section = self._format_compound_section(contract=sub_contract, answer=comparison, index=index)
                publish("\n\n" + section)
            ...
            sub_answer = self._demote_markdown_headings(str(subtask_result.get("answer", "")).strip())
            publish(sub_answer)
            ...
            if isinstance(sub_verification, VerificationReport) and sub_verification.status == "clarify" and isinstance(
                sub_result_contract, QueryContract
            ):
                return self._compound_clarification_response(
                    query=query,
                    session_id=session_id,
                    session=session,
        ...
        answer = "".join(answer_parts).strip()
        self._remember_compound_outcome(session=session, clean_query=clean_query, subtask_results=subtask_results)
    ...
    def _compound_clarification_response(
        self,
        *,
        query: str,
        session_id: str,
        ...
        clarification_options = self._clarification_options(blocked_contract)
        if clarification_options:
            self._store_pending_clarification(session=session, contract=blocked_contract)
        self._remember_clarification_attempt(session=session, contract=blocked_contract, verification=verification)
        ...
        final_answer = prefix + answer if answer else self._clarification_question(blocked_contract, session)
        compound_notes = list(
            dict.fromkeys(
                [
        ...
        return AssistantResponse(
            ...
            verification_report=verification.model_dump(),
            needs_human=True,
            clarification_question=self._clarification_question(blocked_contract, session),
    ...
    def _extract_citation_count_from_evidence(self, *, title: str, evidence: list[EvidenceBlock]) -> dict[str, Any]:
        ...
        for item in evidence:
            ...
            title_overlap = self._title_token_overlap(title, item.title)
            if source != "semantic_scholar" and title_overlap < 0.55:
                continue
            ...
            for pattern in patterns:
                for match in re.finditer(pattern, text, flags=re.I):
                    count = self._parse_citation_count(match.group(1))
                    if count is None:
                        continue
    ...
    def _paper_query_text(contract: QueryContract) -> str:
        ...
        goals = ResearchAssistantAgentV4._research_plan_goals(contract)
    ...
    def _citations_from_doc_ids(self, doc_ids: list[str], evidence: list[EvidenceBlock]) -> list[AssistantCitation]:
        ...
        for doc_id in doc_ids:
            ...
            doc = self.retriever.block_doc_by_id(doc_id) or self.retriever.paper_doc_by_id(lookup_id)
            if doc is None:
                continue
            meta = dict(doc.metadata or {})
            ...
            citations.append(
                AssistantCitation(
                    ...
                    paper_id=str(meta.get("paper_id", lookup_id)),
                    title=str(meta.get("title", "")),
    ...
    def _build_figure_contexts(self, evidence: list[EvidenceBlock], limit: int = 2) -> list[dict[str, Any]]:
        ...
        for item in evidence:
            ...
            if item.block_type == "caption" and item.snippet:
                ...
                group["score"] += 2.0 + (self._figure_signal_score(item.snippet) * 1.5)
            elif item.block_type == "figure" and item.snippet:
                group["figure_parts"].append(item.snippet)
                group["score"] += 3.0 + (self._figure_signal_score(item.snippet) * 1.5)
            ...
            contexts.append(
                {
                    ...
                    "caption": self._join_unique_text(item["caption_parts"], limit=500),
                    "figure_text": self._join_unique_text(item["figure_parts"], limit=500),
                    "page_text": self._join_unique_text(item["page_parts"], limit=700),
                }
            )
        return contexts
