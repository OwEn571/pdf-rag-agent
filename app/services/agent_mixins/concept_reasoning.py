...
class ConceptReasoningMixin:
    def _build_concept_definition_claim(
        self,
        *,
        ...
        llm_payload = self._llm_build_concept_definition(contract=contract, papers=papers, evidence=supporting)
        if llm_payload:
            expansion = str(llm_payload.get("expansion", "") or "").strip()
            category = self._canonicalize_concept_category(str(llm_payload.get("category", "") or "").strip())
            ...
            if definition:
                ...
                confidence = self._coerce_confidence(llm_payload.get("confidence", 0.84))
                return Claim(
        ...
        expansion = self._extract_acronym_expansion(target=target, evidence=supporting)
        category = self._infer_concept_category(target=target, evidence=supporting, expansion=expansion)
        answer = self._compose_concept_definition_text(
    ...
    def _llm_build_concept_definition(
        self,
        *,
        contract: QueryContract,
        papers: list[CandidatePaper],
    ...
    @staticmethod
    def _canonicalize_concept_category(label: str) -> str:
    ...
    @staticmethod
    def _extract_acronym_expansion(*, target: str, evidence: list[EvidenceBlock]) -> str:
    ...
    @staticmethod
    def _compose_concept_definition_text(
        *,
        target: str,
        expansion: str,
        category: str,
