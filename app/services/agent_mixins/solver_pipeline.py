...
class SolverPipelineMixin:
    ...
    def _fallback_goals_from_query(query: str, *, targets: list[str]) -> set[str]:
        ...
        if SolverPipelineMixin._looks_like_metric_goal(query, goals):
            goals.add("metric_value")
        if any(token in normalized for token in ["推荐", "值得", "入门", "recommend"]):
            goals.add("recommended_papers")
        if targets and any(token in query for token in ["是什么", "什么意思", "定义"]) or any(token in normalized for token in ["what is", "what are"]):
        ...
        for item in value:
            ...
            if isinstance(item, dict):
                ...
                if ":" in raw:
                    ...
                    symbol = raw
            symbol = cls._normalize_formula_variable_symbol(symbol)
    ...
    def _normalize_formula_variable_symbol(symbol: str) -> str:
        ...
        normalized = SolverPipelineMixin._normalize_latex_like_math(normalized)
    ...
    def _normalize_extracted_formula_text(cls, text: str) -> str:
        ...
        if not formula:
            return ""
        formula = cls._normalize_formula_label(formula)
        formula = cls._normalize_latex_like_math(formula)
        formula = cls._normalize_formula_label(formula)
    ...
    def _normalize_latex_like_math(cls, text: str) -> str:
        ...
        def replace_gradient_loss(match: re.Match[str]) -> str:
            var = cls._latex_symbol_token(match.group("var"))
        ...
        normalized = re.sub(r"(?:β|\\beta)\s*log(?=\s*(?:π|\\pi|[A-Za-z{_]))", r"\\beta \\log ", normalized)
        replacements = {
            "log σ": r"\log \sigma",
            "logσ": r"\log \sigma",
    ...
    def _formula_block_score(self, text: str, *, contract: QueryContract | None = None) -> float:
        ...
        for token, weight in self.settings.retrieval_formula_token_weights.items():
            if str(token).lower() in haystack:
                score += float(weight)
        if contract is not None and not self._formula_query_wants_gradient(contract.clean_query):
