    ...
    def search_documents(self, query: str, *, limit: int, filter_expr: str | None = None) -> list[Document]:
        ...
        try:
            ...
            if "vector dimension mismatch" in str(exc).lower():
                ...
                logger.warning(
                    "milvus dense search disabled for collection=%s due to embedding dimension mismatch model=%s",
                    self.collection_name,
        ...
        for hit in hits:
            ...
            if isinstance(hit, dict):
                ...
                payload.update({k: v for k, v in hit.items() if k not in {"entity", "vector"}})
