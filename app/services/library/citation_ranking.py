...
from app.services.intents.library import library_query_prefers_previous_candidates
...
CITATION_LOOKUP_STAGE = "citation_count_lookup"
...
def select_citation_ranking_candidates(
    *,
    ...
    def add_candidate(*, title: str, year: str = "", reason: str = "") -> None:
        ...
        # P1-2: Skip titles not in local library — empty paper_id would
        # pollute citation ranking and enable hallucination cross-turn propagation.
        if meta is None:
            return
        selected_keys.add(key)
        selected.append(
            {
                "title": str(meta.get("title", clean_title)),
                "year": str(meta.get("year", year)),
                "paper_id": str(meta.get("paper_id", "")),
                "reason": reason or str(meta.get("generated_summary", "")),
            }
        )
