...
def paper_scope_correction_contract(
    *,
    ...
    # P1-10: Skip prefix if already scoped to same paper (prevent double-prefix)
    scope_prefix = f"限定在论文《{paper.title}》中"
    clean_query = (
        inherited_query
        if scope_prefix in inherited_query or scope_prefix in contract.clean_query
        else f"{scope_prefix}回答：{inherited_query}"
    )
...
def contextual_active_paper_contract(*, contract: QueryContract, paper: CandidatePaper) -> QueryContract:
    ...
    # P1-10: Avoid double-prefix — check if already scoped to any paper
    already_scoped = "限定在论文《" in clean_query and "》中" in clean_query
    if not already_scoped and normalize_entity_key(paper.title) not in normalize_entity_key(clean_query):
...
def promote_contextual_metric_contract(contract: QueryContract) -> QueryContract:
    ...
    notes = list(dict.fromkeys([
        *contract_notes(contract),
        ...
        "answer_slot=metric_value",
        f"original_relation={contract.relation}",  # P1-12: preserve for Runtime Inspector
    ]))
