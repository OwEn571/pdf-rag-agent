...
from app.services.contracts.context import contract_answer_slots, contract_note_values
from app.services.intents.marker_matching import MarkerProfile, normalized_query_text, query_matches_any
from app.services.planning.solver_goals import SOLVER_GOAL_MARKERS, looks_like_metric_goal
...
class DualIndexRetriever:
    ...
    def search_papers(self, *, query: str, contract: QueryContract, limit: int | None = None) -> list[CandidatePaper]:
        ...
        # Title Anchor fallback: inject papers whose body_acronyms match targets
        anchor_docs = self.title_anchor(target_terms) if target_terms else []
        dense_pids = {str((d.metadata or {}).get("paper_id", "")) for d in dense_docs}
        injected = [d for d in anchor_docs if str((d.metadata or {}).get("paper_id", "")) not in dense_pids]
        fused = injected[:3] + dense_docs
    ...
    def _contract_target_alias_terms(contract: QueryContract) -> list[str]:
        terms = contract_note_values(contract, prefix="target_alias=")
        deduped: list[str] = []
        seen: set[str] = set()
        for term in terms:
            key = term.lower()
