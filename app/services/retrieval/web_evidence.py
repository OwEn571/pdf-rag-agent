...
def web_query_text(contract: QueryContract) -> str:
    return web_query_text_from_context(research_plan_context_from_contract(contract))


def web_query_text_from_context(context: ResearchPlanContext) -> str:
    target_text = " ".join(context.targets).strip()
    query = context.clean_query.strip()
...
def web_include_domains(contract: QueryContract) -> list[str]:
    return web_include_domains_from_context(research_plan_context_from_contract(contract))


def web_include_domains_from_context(context: ResearchPlanContext) -> list[str]:
    query = context.clean_query.lower()
    goals = set(context.goals)
    if goals & {"recommended_papers", "followup_papers", "summary", "results", "answer", "general_answer"}:
...
def should_add_web_claim(*, contract: QueryContract, claims: list[Claim], explicit_web: bool) -> bool:
    return should_add_web_claim_from_context(
...
def should_add_web_claim_from_context(
    *,
    ...
    if explicit_web:
        return True
    goals = set(context.goals)
    if allow_web_search and goals & {"answer", "general_answer", "recommended_papers", "followup_papers"}:
        return True
    return not claims
