...
from app.services.contracts.context import contract_answer_slots, contract_note_value, contract_note_values
from app.services.memory.artifacts import remember_conversation_tool_result
from app.services.retrieval.evidence_tools import (
...
from app.services.tools.proposals import propose_tool as record_tool_proposal
from app.services.contracts.session_context import agent_session_conversation_context
from app.services.retrieval.url_fetcher import FetchUrlResult
from app.services.retrieval.web_evidence import merge_evidence
...
def conversation_intent_summary(contract: QueryContract) -> dict[str, Any]:
    intent_kind = contract_note_value(contract, prefix="intent_kind=") or contract.interaction_mode
    return {
        "kind": intent_kind,
...
def read_memory_tool_payload(
    *,
    ...
    context = agent_session_conversation_context(
        session,
...
def conversation_clarification_report(contract: QueryContract) -> dict[str, Any]:
    missing_fields = (
        contract_note_values(contract, prefix="ambiguous_slot=")
        or ["user_choice"]
    )
    return {
        "status": "clarify",
...
def search_corpus_strategy(planned_input: dict[str, Any]) -> str:
    ...
    return strategy if strategy in {"auto", "bm25", "vector", "hybrid"} else "auto"
...
def evidence_event_payload(evidence: list[EvidenceBlock]) -> dict[str, Any]:
    return {"count": len(evidence), "items": [_trim_evidence_item(item.model_dump()) for item in evidence]}


def _trim_evidence_item(payload: dict[str, Any]) -> dict[str, Any]:
    """Trim heavy fields in evidence items for lightweight SSE delivery."""
