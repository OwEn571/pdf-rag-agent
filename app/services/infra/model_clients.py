...
class ModelClients:
    ...
    def http_client(self) -> httpx.Client:
        if self._http_client is None:
            # P1-8: Add timeout to prevent indefinite hanging
        ...
        *,
        ...
        for attempt in range(2):
            try:
                ...
                logger.warning("chat stream_text attempt %d failed: %s", attempt + 1, exc)
                if attempt == 0 and chunks:
                    # Partial stream succeeded; use what we have
                    text = "".join(chunks).strip()
                    return text or fallback
                if attempt == 0:
                    continue  # retry once
        # Both streaming attempts failed; fall back to non-streaming
        logger.warning("chat stream_text falling back to non-streaming invoke")
    ...
    @staticmethod
    def _chat_messages(*, system_prompt: str, messages: list[dict[str, Any]]) -> list[BaseMessage]:
        ...
        for item in messages:
            ...
            if role == "tool":
                ...
                rendered.append(ToolMessage(content=content, tool_call_id=tool_call_id))
