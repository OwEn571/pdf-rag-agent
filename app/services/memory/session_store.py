...
def _trim_context_history(context: SessionContext, *, max_turns: int) -> SessionContext:
    ...
    # P1-7: Clean up expired target_bindings (TTL = 20 turns)
    _expire_old_bindings(context, ttl_turns=20)
...
def _expire_old_bindings(context: SessionContext, *, ttl_turns: int = 20) -> None:
    """Remove target_bindings older than ttl_turns."""
    current_turn = len(context.turns)
    memory = dict(context.working_memory or {})
    ...
    for key, entry in bindings.items():
        if not isinstance(entry, dict):
            continue
        created_at = int(entry.get("created_turn_index", 0) or 0)
        if created_at > 0 and (current_turn - created_at) > ttl_turns:
