...
def remember_learning(*, data_dir: Path, key: str, content: str) -> Path:
    ...
    # P1-11: Hash-dedup before writing — skip if identical content already exists
    content_clean = str(content or "").strip()
    content_hash = hashlib.sha256(content_clean.encode()).hexdigest()[:16]
    if path.exists():
        existing = path.read_text(encoding="utf-8")
        if content_hash in existing:
            return path  # already recorded, skip duplicate
