            ...
            "evidence": [
                {
                    ...
                    "snippet": wrap_untrusted_document_text(item.snippet[:1600], doc_id=item.doc_id, title=item.title),
                }
...
def fallback_formula_payload(
    selected_evidence: list[EvidenceBlock],
    *,
    term_extractor: FormulaTermExtractor,
) -> dict[str, Any]:
    raw = "\n".join(item.snippet[:1200] for item in selected_evidence)
    normalized = normalize_formula_text(raw)
    formula_text = normalize_extracted_formula_text(best_formula_window(normalized))
    evidence_ids = [item.doc_id for item in selected_evidence[:3]]
    return {
        "formula_text": formula_text,
        "formula_latex": formula_text if looks_like_latex_formula(formula_text) else "",
        "evidence_ids": evidence_ids,
        "terms": term_extractor(formula_text or raw),
        "variables": [],
        "formula_format": "latex" if looks_like_latex_formula(formula_text) else "text",
...
def formula_claim_from_payload(
    *,
    ...
    evidence_ids = payload_evidence_ids or [item.doc_id for item in formula_blocks[:3]] or fallback_evidence_ids
    variables = [
        item
    ...
    formula_terms = list(
        dict.fromkeys(
            [
                ...
                *formula_terms_from_variables(variables, term_extractor=term_extractor),
                *term_extractor(formula_text or fallback_term_text),
            ]
        )
    )
    formula_format = str(
        formula_payload.get("formula_format") or ("latex" if looks_like_latex_formula(formula_text) else "text")
    )
    ...
    return Claim(
        ...
        confidence=coerce_confidence_value(formula_payload.get("confidence", 0.82), default=0.82),
    )
